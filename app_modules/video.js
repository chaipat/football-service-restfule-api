...
var videoModule = videoModule.prototype;

function videoModule() {

}

videoModule.getHilightClipVideo = function(req, res, next) {
  ...
  var cacheKey = cacheKeyPrefix + 'main-video-getHilight-clipVideo';
      ...
      if (error) {
        ...
        if(reply) {
          ...
          redisCaching.getCache(res, cacheKey, function(error, result) {
            ...
            // get data from mysql;
            var query = 'SELECT hn.news_id, hn.types, hn.title, hn.thumbnail, sv.lastupdate_date, \
            ...
            ON hn.news_id = sv.video_id2 \
            LEFT JOIN ss_picture hp\
            ON hn.news_id = hp.ref_id \
            WHERE hn.highlight_category_id = 5 \
            AND hp.ref_type = 4\
            AND hn.status = 1 \
