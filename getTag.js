...
function printOut(res, data_list, param_page, param_page_limit) {
	...
	var jsonObj = utils.getJSONPaginationObject(200, "Cache", output, page, page_total, row);
...
function setTagClip(redisCluster, memcached, mysql_connection, key, item, data_list, callback){
	
	var query = "select ";
	query += "`ss_video`.video_id2 as id, `ss_video`.title, `ss_video`.caption as description, ";
	query += "`ss_picture`.folder, `ss_picture`.file_name, concat(`ss_picture`.folder,'/', `ss_picture`.file_name) as thumbnail, ";	
	query += "DATE_FORMAT(`ss_video`.lastupdate_date, '%Y-%m-%d %H:%i:%s') AS`lastupdate_date`, DATE_FORMAT(`ss_video`.create_date, '%Y-%m-%d %H:%i:%s') AS create_date, redirect_url, ";
	...
	query += " LEFt JOIN `ss_tag`ON `ss_tag_pair_clip`.`tag_id` = `ss_tag`.`tag_id` AND `ss_tag`.`status` = 1 ";
	query += " JOIN `ss_video`ON `ss_tag_pair_clip`.`ref_id` = `ss_video`.`video_id2` ";
	...
	query += " LEFT JOIN `ss_picture` ON `ss_video`.`video_id2` = `ss_picture`.`ref_id` AND `ss_picture`.`ref_type` = 4 ";
	query += " WHERE `ss_tag`.`tag_id` = ? AND (`ss_video`.status=1 AND `ss_video`.approve=1) ";
...
function setTagColumn(redisCluster, memcached, mysql_connection, key, item, data_list, callback){
	
	var query = "SELECT `ss_tag_pair_column`.`ref_id` as id, `ss_column`.icon_pic, `ss_column`.icon_vdo, `ss_column`.title,  ";
	query += " DATE_FORMAT(`ss_column`.lastupdate_date, '%Y-%m-%d %H:%i:%s') AS lastupdate_date, DATE_FORMAT(`ss_column`.create_date, '%Y-%m-%d %H:%i:%s') AS create_date, `ss_picture`.folder, `ss_picture`.file_name , redirect_url, ";
	query += " ss_tournament.tournament_id, ss_tournament.tournament_name_th, ss_tournament.tournament_name_en, ss_tournament.domain, ";
	query += " ss_sport.sport_name_th, ss_sport.sport_name_en ";
	query += " FROM `ss_tag_pair_column` JOIN `ss_column` ON `ss_tag_pair_column`.`ref_id` = `ss_column`.`column_id2` AND `ss_column`.`status` = 1 AND `ss_column`.`approve` = 1 ";
	...
	mysql_connection.query({
		...
		if (error) {
			...
			if( !utils.isEmptyObject(result) && result.length > 0 ) {	
				...
				callback(null, redisCluster, memcached, mysql_connection, key, item, data_list);
			} else {
				callback(null, redisCluster, memcached, mysql_connection, key, item, data_list);
			}
		}
...
function setTagNews(redisCluster, memcached, mysql_connection, key, item, callback){
	
	var query = "SELECT `ss_tag_pair_news`.`ref_id` as id, `ss_news`.icon_pic, `ss_news`.icon_vdo, `ss_news`.title,  `ss_news`.countview, `ss_news`.share, ";
	query += " DATE_FORMAT(`ss_news`.lastupdate_date, '%Y-%m-%d %H:%i:%s') AS lastupdate_date, ";
	query += " DATE_FORMAT(`ss_news`.create_date, '%Y-%m-%d %H:%i:%s') AS create_date, `ss_picture`.folder, `ss_picture`.file_name, redirect_url, ";
	query += " ss_tournament.tournament_id, ss_tournament.tournament_name_th, ss_tournament.tournament_name_en, ss_tournament.domain, ";
	query += " ss_sport.sport_name_th, ss_sport.sport_name_en ";
	query += " FROM `ss_tag_pair_news` JOIN `ss_news` ON `ss_tag_pair_news`.`ref_id` = `ss_news`.`news_id2` AND `ss_news`.`status` = 1 AND `ss_news`.`approve` = 1 ";
...
function getTag(redisCluster, memcached, mysql_connection, key, keyword, callback){
	...
	mysql_connection.query({
		...
		if (error) {
			...
			if( !utils.isEmptyObject(result) && result.length > 0 ) {
				callback(null, redisCluster, memcached, mysql_connection, key, result[0]);
			} else {
...
function prepareWaterfall(res, redisCluster, memcached, key, keyword, page, page_limit, cache) {	
	...
	mysql_connection.connect(function(err) {
		if (err) {
			...
			var method_arr = [];
			method_arr.push(async.apply(getTag, redisCluster, memcached, mysql_connection, key, keyword));
			...
			if (err) {
				...
				if(reply == true) {
					if(clear_cache == true) {
						...
						prepareWaterfall(res, redisCluster, null, key, keyword, page, page_limit, clear_cache);
					} else {
