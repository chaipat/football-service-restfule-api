...
var redis_key = config.getKeyType() + config.getKeyPrefix() + config.getKeyProjectName() + 'list-gallery-all-';
...
function getList(mysql_connection, param, key, redisCluster, callback) {
    ...
    var query = "SELECT count(gallery_id2) as row FROM ss_gallery WHERE (status=1 AND approve=1 AND lang='th') ";
    mysql_connection.query({
        ...
        if (error) {
            log.error("[500] list/gallery/getList/getListAllGallery[getList]: " + error.stack);
            ...
            query += " WHERE gallery.lang = 'th' AND gallery.status = 1 AND gallery.approve = 1 ";
            query += " ORDER BY gallery.`order_by` ASC, gallery.`lastupdate_date` DESC ";
...
function getDataFromMySQL(res, redisCluster, param, key) {
    ...
    mysql_connection.connect(function(connectionError) {
        ...
        if (connectionError) {
            ...
            log.error("[500] list/gallery/getList/getListAllGallery Service[getDataFromMySQL]: " + connectionError.stack);
                ...
                if (reply == true) {
                    if (param.clear_cache == true) {
                        ...
                        redisCluster.get(key, function(err, reply) {
                            if (err) {
                                log.error("[500] list/gallery/getList/getListAllGallery Service[redisCluster.get]: " + err.stack);
