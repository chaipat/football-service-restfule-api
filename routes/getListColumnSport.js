...
var redis_key = config.getKeyType() + config.getKeyPrefix() + config.getKeyProjectName() + 'list-column-sport-';
...
function getList(mysql_connection, param, key, redisCluster, callback) {
    ...
    var query = "SELECT count(column_id2) as row FROM ss_column WHERE (status=1 AND approve=1 AND lang='th') and sport_id = " + param.sport_id;
    mysql_connection.query({
        ...
        if (error) {
            log.error("[500] list/column/getList/getListColumnSport[getList]: " + error.stack);
            ...
            query += " LEFT JOIN `ss_picture` AS picture ON a.`column_id2` = picture.`ref_id` ";
            ...
            query += " WHERE a.`sport_id` = " + param.sport_id +" AND a.`approve` = 1 AND a.`status` = 1 AND a.`lang` = 'th' ";
            ...
            mysql_connection.query({
                ...
                if (error) {
                    ...
                    if (utils.isEmptyObject(results)) {
                        ...
                        var jsonObj = utils.getJSONPaginationCustomObject(200, "success", results, param.page, page_total, row, tournament_name_th, tournament_name_en, sport_name_th, sport_name_en, "appListColumnSport");
...
function getDataFromMySQL(res, redisCluster, param, key) {
    ...
    mysql_connection.connect(function(connectionError) {
        ...
        if (connectionError) {
            ...
            log.error("[500] list/column/getList/getListColumnSport Service[getDataFromMySQL]: " + connectionError.stack);
                ...
                if (reply == true) {
                    if (param.clear_cache == true) {
                        ...
                        redisCluster.get(key, function(err, reply) {
                            if (err) {
                                log.error("[500] list/column/getList/getListColumnSport Service[redisCluster.get]: " + err.stack);
