...
var redis_key = config.getKeyType() + config.getKeyPrefix() + config.getKeyProjectName() + 'list-column-tournament-';
...
function getList(mysql_connection, param, key, redisCluster, callback) {
    ...
    var query = "SELECT count(column_id2) as row FROM ss_column WHERE (status=1 AND approve=1 AND lang='th') and tournament_id = " + param.tournament_id;
    mysql_connection.query({
        ...
        if (error) {
            log.error("[500] list/column/getList/getListColumnTournament[getList]: " + error.stack);
            ...
            query += " WHERE a.`tournament_id` = " + param.tournament_id +" ";
            query += " AND a.`status` = 1 AND a.`lang` = 'th' ";
            ...
            mysql_connection.query({
                ...
                if (error) {
                    ...
                    var resultObject2 = results[0];
                        ...
                        var jsonObj = utils.getJSONPaginationCustomObject(200, "success", results, param.page, page_total, row, tournament_name_th, tournament_name_en, sport_name_th, sport_name_en, "appListColumnTournament");
...
function getDataFromMySQL(res, redisCluster, param, key) {
    ...
    mysql_connection.connect(function(connectionError) {
        ...
        if (connectionError) {
            ...
            log.error("[500] list/column/getList/getListColumnTournament Service[getDataFromMySQL]: " + connectionError.stack);
                ...
                if (reply == true) {
                    if (param.clear_cache == true) {
                        ...
                        redisCluster.get(key, function(err, reply) {
                            if (err) {
                                log.error("[500] list/column/getList/getListColumnTournament Service[redisCluster.get]: " + err.stack);
