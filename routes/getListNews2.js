...
var redis_key = config.getKeyType() + config.getKeyPrefix() + config.getKeyProjectName() + 'list-news-tournmant-';
...
function getList(mysql_connection, param, key, redisCluster, callback) {
    ...
    var query = "SELECT count(news_id) as row FROM ss_news WHERE (status=1 AND approve=1 AND lang='th') and tournament_id = " + param.tournament_id;
    mysql_connection.query({
        ...
        if (error) {
            log.error("[500] list/news/getList/tournament[getList]: " + error.stack);
            ...
            query = "SELECT news_id2 as id, ss_news.tournament_id, ss_news.sport_id, ";
            query += " icon_pic, icon_vdo, ss_news.news_special_id, ss_ns.name as news_special_name, headline, title, countview, share, ";
            ...
            query += " ss_tournament.tournament_name_en, ss_tournament.url as tournment_url, ss_tournament.dimension as tournment_dimension, ss_tournament.domain as domain, ";
            ...
            query += " AND (ss_news.lang='th' AND `ss_news`.`tournament_id` = " + param.tournament_id + ") ";
            ...
            mysql_connection.query({
                ...
                if (error) {
                    log.error("[500] list/news/getList/tournment[getList]: " + error.stack);
                    callback(500, error.stack);
                    throw new Error(error.stack);
                } else {
...
function getDataFromMySQL(res, redisCluster, param, key) {
    ...
    mysql_connection.connect(function(connectionError) {
        ...
        if (connectionError) {
            ...
            log.error("[500] list/news/getList/getListNews2 Service[getDataFromMySQL]: " + connectionError.stack);
...
function getDataFromRedis(res, param) {
    ...
    var key = redis_key + param.tournament_id + "-" + param.limit + "-" + param.page;
                ...
                if (reply == true) {
                    if (param.clear_cache == true) {
                        ...
                        redisCluster.get(key, function(err, reply) {
                            if (err) {
                                log.error("[500] list/news/getList/getListNews2 Service[redisCluster.get]: " + err.stack);
...
router.get('/:tournament_id/:item/:page', function(req, res, next) {
    var param = {};
    param.tournament_id = req.params.tournament_id;
...
router.get('/:tournament_id/:item/:page/:clear_cache', function(req, res, next) {
    var param = {};
    param.tournament_id = req.params.tournament_id;
    param.limit = req.params.item;
    param.page = req.params.page;
