...
function getRelateClip(redisCluster, mysql_connection, news_data, callback) {
	...
	var tournament_id = news_data.tournament_id;
	var data = [];
	data.push(tournament_id);
	...
	query += " tournament.tournament_name_th, tournament.tournament_name_en, tournament.url as tournament_url, tournament.dimension as tournament_dimension, ss_tournament.domain as domain, ";
	...
	query += " WHERE vdo.tournament_id = ? AND vdo.lastupdate_date >= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY) AND vdo.lastupdate_date <= CURRENT_DATE AND vdo.status=1 AND vdo.approve=1 AND vdo.lang='th' ";
	...
	mysql_connection.query({
		...
		if (error) {
			log.error( "[500] getListTopview7DaysTournament Service: [getRelateClip]: " + error.stack );
		} else {
...
function getRelateColumn(redisCluster, mysql_connection, news_data, callback) {
	...
	var query = " SELECT a.`column_id2` as id, a.`match_id`, a.`profile_id`, ";
			...
			query += " WHERE a.tournament_id = ? AND a.lastupdate_date >= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY) ";
	...
	mysql_connection.query({
		...
		if (error) {
			log.error( "[500] getListTopview7DaysTournament Service: [getRelateColumn]: " + error.stack );
		} else {
...
function getRelate(redisCluster, mysql_connection, news_data, callback) {
	...
	var query = " SELECT news_id2 as id, ss_news.tournament_id, ss_news.sport_id, ss_news.icon_pic, ";
		query += " icon_vdo, ss_news.news_special_id, ss_ns.name as news_special_name, headline, ss_news.title, ss_news.countview, ss_news.share, ";
		...
		query += " WHERE ss_news.tournament_id = ? AND ss_news.lastupdate_date >= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY) AND ss_news.lastupdate_date <= CURRENT_DATE AND ss_news.status=1 AND ss_news.approve=1 AND ss_news.lang='th' ";
	...
	mysql_connection.query({
		...
		if (error) {
			log.error( "[500] getListTopview7DaysTournament Service: [getRelate]: " + error.stack );
			news_data.topview = [];
		} else {
...
function prepareWaterfall(res, redisCluster, param) {
	...
	mysql_connection.connect(function(err) {
		...
		if (err) {
			...
			log.error( "[500] getListTopview7DaysTournament Service: [prepareWaterfall]: " + err.stack );
				...
				if(reply == true){
					...
					if(clear_cache == true) {
						...
						redisCluster.get( key, function (error, reply) {
							if(error) {
								log.error( "[500] getListTopview7DaysTournament Service: [RedisCluster.get]: " + error.stack );
...
router.get('/:tournament_id', function(req, res, next) {
	
	var tournament_id = req.params.tournament_id;
	var param = {};

	param.key = cache_key + "_tournament_" + tournament_id;
	param.cache = false;	
...
router.get('/:tournament_id/:clear_cache', function(req, res, next) {
