...
var redis_key = config.getKeyType() + config.getKeyPrefix() + config.getKeyProjectName() + 'list-video-tournament-';
...
function getList(mysql_connection, param, key, redisCluster, callback) {
    ...
    var query = "SELECT count(video_id2) as row FROM ss_video WHERE (status=1 AND approve=1 AND lang='th') and tournament_id = " + param.video_tournament_id;
    mysql_connection.query({
        ...
        if (error) {
            log.error("[500] list/video/getList/getListVideoTournament[getList]: " + error.stack);
            ...
            query += " AND vdo.`tournament_id` = " + param.video_tournament_id + " ORDER BY vdo.`order_by` ASC, vdo.`lastupdate_date` DESC ";
            ...
            mysql_connection.query({
                ...
                if (error) {
                    ...
                    if (utils.isEmptyObject(results)) {
                        ...
                        var jsonObj = utils.getJSONPaginationCustomObject(200, "success", results, param.page, page_total, row, tournament_name_th, tournament_name_en, sport_name_th, sport_name_en, "appListVideoTournament");
...
function getListFromPhp(mysql_connection, param, key, redisCluster, callback) {
    var video_id = param.video_tournament_id;
    var page = param.page;

    request('http://sstv.siamsport.co.th/rss/list_video.php?page=' + page + '&idtss=' + video_id, function(error, response, body) {
...
function getDataFromMySQL(res, redisCluster, param, key) {
    ...
    mysql_connection.connect(function(connectionError) {
        ...
        if (connectionError) {
            ...
            log.error("[500] list/video/getList/getListVideoTournament Service[getDataFromMySQL]: " + connectionError.stack);
                ...
                if (reply == true) {
                    if (param.clear_cache == true) {
                        ...
                        redisCluster.get(key, function(err, reply) {
                            if (err) {
                                log.error("[500] list/video/getList/getListVideoTournament Service[redisCluster.get]: " + err.stack);
