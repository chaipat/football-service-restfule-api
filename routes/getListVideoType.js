...
var redis_key = config.getKeyType() + config.getKeyPrefix() + config.getKeyProjectName() + 'list-video-type-';
...
function getList(mysql_connection, param, key, redisCluster, callback) {
    ...
    var query = "SELECT count(video_id2) as row FROM ss_video WHERE (status=1 AND approve=1 AND lang='th') and video_type_id = " + param.video_type_id;
    mysql_connection.query({
        ...
        if (error) {
            log.error("[500] list/video/getList/getListVideoType[getList]: " + error.stack);
            ...
            query += " sport.sport_name_en, sport.sport_name_th, sport.url as sport_url, sport.dimension as sport_dimension, vt.video_type_name ";
            ...
            query += " LEFT JOIN ss_video_type vt ON vdo.video_type_id = vt.video_type_id2 ";
            query += " WHERE (vdo.`status` = 1 AND vdo.`approve` = 1) ";
            query += " AND vdo.`lang` = 'th' AND vdo.`video_type_id` = " + param.video_type_id + " ORDER BY vdo.`order_by` ASC, vdo.`lastupdate_date` DESC ";
            ...
            mysql_connection.query({
                ...
                if (error) {
                    ...
                    if (utils.isEmptyObject(results)) {
                        ...
                        var video_type_name = resultObject2["video_type_name"];

                        var jsonObj = utils.getJSONPaginationCustomObject(200, "success", results, param.page, page_total, row, null, null, null, null, video_type_name, "appListVideoType");
                        ...
                        var jsonObj = utils.getJSONPaginationCustomObject(200, "Success", results, param.page, page_total, row, null, null, null, null, video_type_name);
...
function getDataFromMySQL(res, redisCluster, param, key) {
    ...
    mysql_connection.connect(function(connectionError) {
        ...
        if (connectionError) {
            ...
            log.error("[500] list/video/getList/getListVideoType Service[getDataFromMySQL]: " + connectionError.stack);
...
function getDataFromRedis(res, param) {
    ...
    var key = redis_key + param.video_type_id + "-" + param.limit + "-" + param.page;
                ...
                if (reply == true) {
                    if (param.clear_cache == true) {
                        ...
                        redisCluster.get(key, function(err, reply) {
                            if (err) {
                                log.error("[500] list/video/getList/getListVideoType Service[redisCluster.get]: " + err.stack);
...
router.get('/:video_type_id/:item/:page', function(req, res, next) {
    var param = {};
    param.video_type_id = req.params.video_type_id;
...
router.get('/:video_type_id/:item/:page/:clear_cache', function(req, res, next) {
    var param = {};
    param.video_type_id = req.params.video_type_id;
    param.limit = req.params.item;
    param.page = req.params.page;
