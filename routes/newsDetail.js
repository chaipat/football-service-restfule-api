...
function getCreditName(
  ...
  var query =
    "SELECT credit_name FROM ss_credit WHERE credit_id IN (" +
    jsonData.credit_id +
    ") ";
  mysql_connection.query(
    ...
    function(error, result) {
      if (error) {
        log.error(
          "[500] detail/news/getDetail Service[getCreditName]: " + error.stack
        );
...
function getTag(
  ...
  query =
    query +
    " LEFT JOIN `ss_tag` on `ss_tag`.tag_id = `ss_tag_pair_news`.tag_id ";
...
function getDetail(mysql_connection, param, key, redisCluster, callback) {
  ...
  var query_string =
    " SELECT news.news_id2 as news_id, news.news_special_id, ss_ns.name as news_special_name, news.icon_pic, news.icon_vdo, news.rate18_flag,";
  query_string +=
    " news.tournament_id, tournament.tournament_name_th, tournament.tournament_name_en, tournament.url as tournament_url, tournament.dimension as tournament_dimension, news.redirect_url,";
  query_string += " tournament.domain AS domain, ";
  query_string +=
    " news.sport_id, sport.sport_name_th, sport.sport_name_en, sport.url as sport_url, sport.dimension as sport_dimension, news.headline, ";
  query_string += " news.title, news.description, news.detail, ";
  query_string +=
    " DATE_FORMAT(news.create_date, '%d-%m-%Y %H:%i') AS create_date, ";
  query_string +=
    " DATE_FORMAT(news.lastupdate_date, '%d-%m-%Y %H:%i') AS lastupdate_date, ";
  query_string +=
    " DATE_FORMAT(news.proof_date, '%d-%m-%Y %H:%i') AS proof_date, ";
  query_string +=
    " DATE_FORMAT(news.approve_date, '%d-%m-%Y %H:%i') AS approve_date, ";
  query_string +=
    " news.credit_id, picture.folder, picture.file_name, picture.caption , news.factcheck";
  query_string += " FROM `ss_news` AS news LEFT JOIN `ss_picture` AS picture ";
  query_string +=
    " ON news.news_id2 = picture.ref_id and picture.ref_type = 1 and picture.default = 1 ";
  query_string +=
    " LEFT JOIN ss_tournament tournament ON news.tournament_id = tournament.tournament_id ";
  query_string +=
    " LEFT JOIN ss_sport sport ON news.sport_id = sport.sport_id ";
  query_string +=
    " LEFT JOIN ss_news_special ss_ns ON news.news_special_id = ss_ns.news_special_id ";
  if (param.preview) {
    query_string += " WHERE news.`lang` = 'th' AND news.`news_id2` = ? ";
  } else {
    query_string +=
      " WHERE news.`status` = 1 AND news.`approve` = 1 AND news.`lang` = 'th' AND news.`news_id2` = ? ";
  }
  ...
  try {
    mysql_connection.query(
      ...
      function(error, result) {
        if (error) {
          log.error(
            "[500] detail/news/getDetail Service[getDetail]: " + error.stack
          );
          callback(500, error.stack);
        } else {
          if (!utils.isEmptyObject(result) && result.length > 0) {
            ...
            for (var i in data2) {
              ...
              var picture_size = {
                fullsize:
                  ...
                  data2[i]["file_name"],
                size128:
                  "size128/" + data2[i]["folder"] + "/" + data2[i]["file_name"],
                size224:
                  "size224/" + data2[i]["folder"] + "/" + data2[i]["file_name"],
                size304:
                  "size304/" + data2[i]["folder"] + "/" + data2[i]["file_name"],
                size640:
                  "size640/" + data2[i]["folder"] + "/" + data2[i]["file_name"]
              };
...
function getDataFromMySQL(res, redisCluster, param, key) {
  ...
  mysql_connection.connect(function(connectError) {
    if (connectError) {
      ...
      log.error(
        "[500] detail/news/getDetail Service[getDataFromMySQL]: " +
          connectError.stack
      );
        ...
        function(error, result) {
          ...
          if (error) {
            ...
            if (error == 200) {
              utils.printJSON(
                res,
                utils.getJSONObject(200, responseStatus, output)
              );
            } else {
              if (redisCluster != null) {
                redisCluster.disconnect();
        ...
        if (err) {
          ...
          if (reply == true) {
            if (param.clear_cache == true) {
              ...
              utils.printJSON(
                res,
                utils.getJSONObject(200, "Delete: " + key, null)
              );
            } else {
              redisCluster.get(key, function(error, reply) {
                if (error) {
                  log.error(
                    "[500] detail/news/getDetail Service[redisCluster.get]: " +
                      error.stack
                  );
                  getDataFromMySQL(res, redisCluster, param, key);
                } else {
                  if (reply != "" && reply != undefined) {
                    ...
                    utils.printJSON(
                      res,
                      utils.getJSONObject(200, "Redis", json)
                    );
                  } else {
                    getDataFromMySQL(res, redisCluster, param, key);
                  }
