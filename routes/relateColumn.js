...
function getRelateClip(redisCluster, mysql_connection, news_data, callback) {
	...
	var query = "SELECT `ss_column_relate`.order, `ss_column_relate`.news_id as relate_id, `ss_video`.title, concat(`ss_picture`.`folder`,'/',`ss_picture`.`file_name`) as picture, `ss_picture`.`folder`,`ss_picture`.`file_name`, redirect_url, DATE_FORMAT(`ss_video`.`lastupdate_date`, '%d-%m-%Y %H:%m') AS lastupdate_date, ss_tournament.tournament_id, ss_tournament.tournament_name_th, ss_tournament.tournament_name_en, ss_tournament.url as tournament_url, ss_tournament.dimension as tournament_dimension, ss_tournament.domain as domain, ss_sport.sport_name_th, ss_sport.sport_name_en, ss_sport.url as sport_url, ss_sport.dimension as sport_dimension "; 
	query = query + " FROM `ss_column_relate` ";
	query = query + " JOIN `ss_video` ON `ss_column_relate`.`news_id` = `ss_video`.`video_id2` ";
	query = query + " AND `ss_video`.`status` = 1 AND `ss_video`.`approve` = 1 and `ss_column_relate`.type = 'clip'";
	...
	query = query + " WHERE `ss_column_relate`.ref_id = ? " ;		
	query = query + " ORDER BY `ss_column_relate`.`order` ASC LIMIT 4 ";
...
function getRelateColumn(redisCluster, mysql_connection, news_data, callback) {
	...
	var id = news_data["id"];
	var relate = news_data["relate"];
	var query = "SELECT `ss_column_relate`.order, `ss_column_relate`.news_id as relate_id, ";
	query = query + " ss_news.news_special_id, ss_ns.name as news_special_name, `ss_news`.title, ss_news.icon_vdo, concat(`ss_picture`.`folder`,'/',`ss_picture`.`file_name`) as picture, ";
	query = query + " redirect_url, DATE_FORMAT(`ss_news`.`lastupdate_date`, '%d-%m-%Y %H:%m') AS lastupdate_date, ss_tournament.tournament_id, ss_tournament.tournament_name_th, ss_tournament.tournament_name_en, ss_tournament.url as tournament_url, ss_tournament.dimension as tournamnet_dimension, ss_tournament.domain as domain, ss_sport.sport_name_th, ss_sport.sport_name_en, ss_sport.url as sport_url, ss_sport.dimension as sport_dimension, "; 
	query = query + " ss_picture.folder, ss_picture.file_name ";
	query = query + " FROM `ss_column_relate` ";
	query = query + " JOIN `ss_news` ON `ss_column_relate`.`news_id` = `ss_news`.`news_id2` ";
	query = query + " AND `ss_news`.`status` = 1 AND `ss_news`.`approve` = 1 and `ss_column_relate`.type = 'news'";
	...
	query = query + " AND `ss_news`.`status` = 1 AND `ss_news`.`approve` = 1 ";
	...
	mysql_connection.query({
		...
		if (error) {
			log.error( "[500] getRelateColumn Service: " + error );
		} else {
...
function getRelate(redisCluster, mysql_connection, news_data, callback) {
	...
	var id = news_data["id"];
	var query = "SELECT `ss_column_relate`.order, `ss_column_relate`.news_id as relate_id, \
	 `ss_column`.title, ss_column.icon_vdo, concat(`ss_picture`.`folder`,'/',`ss_picture`.`file_name`) as picture, `ss_picture`.`folder`, `ss_picture`.`file_name`, redirect_url, DATE_FORMAT(`ss_column`.`lastupdate_date`, '%d-%m-%Y %H:%m') AS lastupdate_date, ss_tournament.tournament_id, ss_tournament.tournament_name_th, ss_tournament.tournament_name_en, ss_tournament.url as tournament_url, ss_tournament.dimension as tournament_dimension, ss_tournament.domain as domain, ss_sport.sport_name_th, ss_sport.sport_name_en, ss_sport.url as sport_url, ss_sport.dimension as sport_dimension "; 
	query = query + " FROM `ss_column_relate` ";
	query = query + " JOIN `ss_column` ON `ss_column_relate`.`news_id` = `ss_column`.`column_id2` ";
	query = query + " AND `ss_column`.`status` = 1 AND `ss_column`.`approve` = 1 AND `ss_column_relate`.type = 'column' ";
	...
	query = query + " AND `ss_column`.`status` = 1 AND `ss_column`.`approve` = 1 ";
	query = query + " WHERE `ss_column_relate`.ref_id = ? " ;		
	query = query + " ORDER BY `ss_column_relate`.`order` ASC LIMIT 4 ";
				...
				if(reply == true){
					...
					if(clear_cache == true) {
						...
						redisCluster.get( key, function (error, reply) {
							if(error) {
								log.error( "[500] RelateColumn Service: " + error );
